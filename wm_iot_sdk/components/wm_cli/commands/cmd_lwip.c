...
static void show_netif_info(struct netif *netif)
{
    ...
    wm_cli_printf("flags=%x<%s%s%s%s>  mtu %hu\r\n", netif->flags, netif->flags & NETIF_FLAG_LINK_UP ? "UP" : "DOWN",
                  netif->flags & NETIF_FLAG_BROADCAST ? " BROADCAST" : "",
                  netif->flags & NETIF_FLAG_UP ? " RUNNING" : " STOPPED", netif->flags & NETIF_FLAG_IGMP ? " MULTICAST" : "",
                  netif->mtu);
...
static void cmd_ifconfig(int argc, char *argv[])
{
    ...
    {
        if (argc >= 2) {
            if ((3 == strlen(argv[1])) && (argv[1][0] == netif->name[0]) && (argv[1][1] == netif->name[1]) &&
                ((argv[1][2] - '0') == netif->num)) {
                if (argc == 5) {
#if LWIP_IPV4
...
#if LWIP_IPV4
                    if (!err)
                        err = netifapi_netif_set_addr(netif, (const ip4_addr_t *)&ipaddr, (const ip4_addr_t *)&netmask,
                                                      (const ip4_addr_t *)&gw);
#endif
                    if (!err)
...
WM_CLI_CMD_DEFINE(ifconfig, cmd_ifconfig, ifconfig cmd,
                  ifconfig[if ip mask gw]-- show / set netif info); //cppcheck # [syntaxError]