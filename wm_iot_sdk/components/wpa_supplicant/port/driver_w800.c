...
#if defined(CONFIG_WPA_SUPPLICANT_LOG_LEVEL_INFO) || defined(CONFIG_WPA_SUPPLICANT_LOG_LEVEL_DEBUG) || \
    defined(CONFIG_WPA_SUPPLICANT_LOG_LEVEL_MSGDUMP) || defined(CONFIG_WPA_SUPPLICANT_LOG_LEVEL_EXCESSIVE)
        drv_cfg.show_info = 1;
#endif
...
static struct hostapd_hw_modes *wm_wifi_driver_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags, u8 *dfs)
{
    int i;
    ...
    modes->channels     = os_zalloc(modes->num_channels * sizeof(struct hostapd_channel_data));
    if (!modes->channels) {
        os_free(modes);
        return NULL;
    }
...
static int wm_wifi_driver_get_capa(void *priv, struct wpa_driver_capa *capa)
{
    ...
    capa->key_mgmt = WPA_DRIVER_CAPA_KEY_MGMT_WPA | WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK | WPA_DRIVER_CAPA_KEY_MGMT_WPA2 |
...
#ifdef CONFIG_AP
                  WPA_DRIVER_FLAGS_AP | WPA_DRIVER_FLAGS_AP_UAPSD | WPA_DRIVER_FLAGS_AP_MLME |
#endif
#ifdef CONFIG_SAE
                  WPA_DRIVER_FLAGS_SAE |
#endif
                  WPA_DRIVER_FLAGS_HT_2040_COEX | WPA_DRIVER_FLAGS_EAPOL_TX_STATUS;
...
#ifdef CONFIG_SAE
static int wm_wifi_driver_send_external_auth_status(void *priv, struct external_auth *params)
{
...
static int wm_wifi_driver_send_action(void *priv, unsigned int freq, unsigned int wait, const u8 *dst, const u8 *src,
...
static int wm_wifi_driver_send_mlme(void *priv, const u8 *data, size_t data_len, int noack, unsigned int freq,
...
static int wm_wifi_driver_hapd_send_eapol(void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt,
...
static int wm_wifi_driver_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
...
static int wm_wifi_driver_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, u16 reason)
{
    struct ieee80211_mgmt *hdr;
    u8 *buf;
