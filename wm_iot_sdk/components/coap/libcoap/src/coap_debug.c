...
#define fprintf(fd, ...)                                         \
    {                                                            \
        (void)fd;                                                \
        wm_log_printf(WM_LOG_LEVEL_DEBUG, LOG_TAG, __VA_ARGS__); \
    }
#define fflush(...)
#define vfprintf(fd, ...)                                         \
    {                                                             \
        (void)fd;                                                 \
        wm_log_vprintf(WM_LOG_LEVEL_DEBUG, LOG_TAG, __VA_ARGS__); \
    }
...
COAP_STATIC_INLINE size_t print_timestamp(char *s, size_t len, coap_tick_t t)
{
    ...
    if (len > lensofar + 4) {
        lensofar += snprintf(&s[lensofar], len - lensofar, ".%03u", (unsigned int)((coap_ticks_to_rt_us(t) % 1000000) / 1000));
    }
    return lensofar;
}
...
{
#ifdef HAVE_SNPRINTF
    return snprintf(s, len, "%u.%03u", (unsigned int)coap_ticks_to_rt(t),
...
size_t coap_print_addr(const coap_address_t *addr, unsigned char *buf, size_t len)
{
    ...
    switch (addr->addr.sa.sa_family) {
        ...
        case AF_INET:
            snprintf((char *)buf, len, "%s:%d", coap_print_ip_addr(addr, scratch, sizeof(scratch)),
    ...
    snprintf((char *)buf, len, "[%s]:%d", coap_print_ip_addr(addr, scratch, sizeof(scratch)), coap_address_get_port(addr));
    return strlen((char *)buf);
#else /* HAVE_SNPRINTF */
    unsigned char *p = buf;
...
#warning "IPv4 network addresses will not be included in debug output"
    ...
    *p   = '\000';
    ...
    snprintf((char *)buf, len, "[%s]:%d", coap_print_ip_addr(addr, scratch, sizeof(scratch)), addr->port);
    return strlen((char *)buf);
#else /* HAVE_SNPRINTF */
    unsigned char *p = buf;
...
{
    return !(content_format == -1 || content_format == COAP_MEDIATYPE_TEXT_PLAIN ||
             content_format == COAP_MEDIATYPE_APPLICATION_LINK_FORMAT || content_format == COAP_MEDIATYPE_APPLICATION_XML ||
...
void coap_show_pdu(coap_log_t level, const coap_pdu_t *pdu)
{
    ...
    if (!pdu->session || COAP_PROTO_NOT_RELIABLE(pdu->session->proto)) {
        snprintf(outbuf, sizeof(outbuf), "v:%d t:%s c:%s i:%04x {", COAP_DEFAULT_VERSION, msg_type_string(pdu->type),
                 msg_code_string(pdu->code), pdu->mid);
    } else if (pdu->session->proto == COAP_PROTO_WS || pdu->session->proto == COAP_PROTO_WSS) {
        if (pdu->type != COAP_MESSAGE_CON)
            coap_log_alert("WebSocket: type != CON\n");
        snprintf(outbuf, sizeof(outbuf), "v:WebSocket c:%s {", msg_code_string(pdu->code));
    } else {
        if (pdu->type != COAP_MESSAGE_CON)
            coap_log_alert("Reliable: type != CON\n");
        snprintf(outbuf, sizeof(outbuf), "v:Reliable c:%s {", msg_code_string(pdu->code));
    }

    for (i = 0; i < pdu->actual_token.length; i++) {
        outbuflen = strlen(outbuf);
        snprintf(&outbuf[outbuflen], sizeof(outbuf) - outbuflen, "%02x", pdu->actual_token.s[i]);
    }
    outbuflen = strlen(outbuf);
    ...
    while ((option = coap_option_next(&opt_iter))) {
        ...
        } else if (pdu->code == COAP_SIGNALING_CODE_PING || pdu->code == COAP_SIGNALING_CODE_PONG) {
            buf_len = 0;
        } else if (pdu->code == COAP_SIGNALING_CODE_RELEASE) {
            switch (opt_iter.number) {
                case COAP_SIGNALING_OPTION_ALTERNATIVE_ADDRESS:
                    buf_len = print_readable(coap_opt_value(option), coap_opt_length(option), buf, sizeof(buf), 0);
                    break;
         ...
         * letter M if set, the _ otherwise */
                    if (COAP_OPT_BLOCK_SZX(option) == 7) {
                        if (coap_get_data(pdu, &data_len, &data))
                            ...
                            buf_len =
                                snprintf((char *)buf, sizeof(buf), "%u/%c/BERT", coap_opt_block_num(option), /* block number */
                                         COAP_OPT_BLOCK_MORE(option) ? 'M' : '_');                           /* M bit */
                    } else {
                        buf_len = snprintf((char *)buf, sizeof(buf), "%u/%c/%u", coap_opt_block_num(option), /* block number */
                                           COAP_OPT_BLOCK_MORE(option) ? 'M' : '_',                          /* M bit */
                ...
                case COAP_OPTION_OSCORE:
                    ...
                    if (opt_len) {
                        ...
                        if (opt_val[0] & 0x07) {
                            ...
                            snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%spIV=0x", buf_len ? "," : "");
                            for (i = 0; (uint32_t)i < cnt; i++) {
                                buf_len = strlen((char *)buf);
                                snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%02x", opt_val[ofs + i]);
                            }
                            ofs += cnt;
                        }
                        if (opt_val[0] & 0x10) {
                            ...
                            ofs++;
                            buf_len = strlen((char *)buf);
                            snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%skc=0x", buf_len ? "," : "");
                            for (i = 0; (uint32_t)i < cnt; i++) {
                                buf_len = strlen((char *)buf);
                                snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%02x", opt_val[ofs + i]);
                            }
                        ...
                        if (opt_val[0] & 0x08) {
                            ...
                            cnt     = opt_len - ofs;
                            buf_len = strlen((char *)buf);
                            snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%skid=0x", buf_len ? "," : "");
                            for (i = 0; (uint32_t)i < cnt; i++) {
                                buf_len = strlen((char *)buf);
                                snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%02x", opt_val[ofs + i]);
                            }
                ...
                case COAP_OPTION_RTAG:
                    ...
                    for (i = 0; (uint32_t)i < opt_len; i++) {
                        buf_len = strlen((char *)buf);
                        snprintf((char *)&buf[buf_len], sizeof(buf) - buf_len, "%02x", opt_val[i]);
                    }
                    buf_len = strlen((char *)buf);
                    break;
                default:
                    /* generic output function for all other option types */
                    if (opt_iter.number == COAP_OPTION_URI_PATH || opt_iter.number == COAP_OPTION_PROXY_URI ||
                        opt_iter.number == COAP_OPTION_URI_HOST || opt_iter.number == COAP_OPTION_LOCATION_PATH ||
                        opt_iter.number == COAP_OPTION_LOCATION_QUERY || opt_iter.number == COAP_OPTION_PROXY_SCHEME ||
        ...
        outbuflen = strlen(outbuf);
        snprintf(&outbuf[outbuflen], sizeof(outbuf) - outbuflen, " %s:%.*s", msg_option_string(pdu->code, opt_iter.number),
                 (int)buf_len, buf);
    }
    ...
    if (coap_get_data(pdu, &data_len, &data)) {
       ...
       */
            ...
            if (max_length > 1) {
                outbuflen += print_readable(data, data_len, (unsigned char *)&outbuf[outbuflen], max_length, 0);
            }
    ...
    switch (tls_version->type) {
        ...
        case COAP_TLS_LIBRARY_TINYDTLS:
            snprintf(buffer, bufsize,
                     ...
                     (unsigned long)(tls_version->version >> 16), (unsigned long)((tls_version->version >> 8) & 0xff),
                     (unsigned long)(tls_version->version & 0xff), (unsigned long)(tls_version->built_version >> 16),
        ...
        case COAP_TLS_LIBRARY_OPENSSL:
            ...
            sub[0] = ((tls_version->version >> 4) & 0xff) ? ((tls_version->version >> 4) & 0xff) + 'a' - 1 : '\000';
            sub[1] = '\000';
            ...
            b_sub[0] =
                ((tls_version->built_version >> 4) & 0xff) ? ((tls_version->built_version >> 4) & 0xff) + 'a' - 1 : '\000';
            b_sub[1] = '\000';
            snprintf(buffer, bufsize,
                     "TLS Library: OpenSSL - runtime "
                     "%lu.%lu.%lu%s%s, libcoap built for %lu.%lu.%lu%s%s",
                     (unsigned long)(tls_version->version >> 28), (unsigned long)((tls_version->version >> 20) & 0xff),
                     ...
                     (unsigned long)((tls_version->built_version >> 12) & 0xff), b_sub, b_beta);
            break;
        ...
        case COAP_TLS_LIBRARY_MBEDTLS:
            snprintf(buffer, bufsize,
                     ...
                     "libcoap built for %lu.%lu.%lu",
                     (unsigned long)(tls_version->version >> 24), (unsigned long)((tls_version->version >> 16) & 0xff),
                     (unsigned long)((tls_version->version >> 8) & 0xff), (unsigned long)(tls_version->built_version >> 24),
...
char *coap_string_tls_support(char *buffer, size_t bufsize)
{
    ...
    snprintf(buffer, bufsize,
             ...
             have_dtls ? "" : "No ", have_tls ? "" : "no ", have_psk ? "" : "no ", have_pki ? "" : "no ",
             have_pkcs11 ? "" : "no ", have_rpk ? "" : "no ", have_oscore ? "Have " : "No ", have_ws ? "Have " : "No ");
    return buffer;
}
...
void coap_log_impl(coap_log_t level, const char *format, ...)
{
    ...
    coap_level = (level == COAP_LOG_EMERG  ? WM_LOG_LEVEL_ERROR :
                  level == COAP_LOG_ALERT  ? WM_LOG_LEVEL_ERROR :
                  level == COAP_LOG_CRIT   ? WM_LOG_LEVEL_ERROR :
                  level == COAP_LOG_ERR    ? WM_LOG_LEVEL_ERROR :
                  level == COAP_LOG_WARN   ? WM_LOG_LEVEL_WARN :
                  level == COAP_LOG_NOTICE ? WM_LOG_LEVEL_WARN :
                  level == COAP_LOG_INFO   ? WM_LOG_LEVEL_INFO :
                  level == COAP_LOG_DEBUG  ? WM_LOG_LEVEL_DEBUG :
                  level == COAP_LOG_OSCORE ? WM_LOG_LEVEL_DEBUG :
                                             WM_LOG_LEVEL_VERBOSE);

    if (format) {
        wm_log_vprintf(coap_level, LOG_TAG, format, list);
    }
    va_end(list);
}
...
int coap_debug_send_packet(void)
{
    ...
    if (num_packet_loss_intervals > 0) {
        ...
        for (i = 0; i < num_packet_loss_intervals; i++) {
            if (send_packet_count >= packet_loss_intervals[i].start && send_packet_count <= packet_loss_intervals[i].end) {
