...
int Log_initialize(Log_nameValue* info)
{
	...
	if ((rc = pthread_mutex_init(log_mutex, &attr)) != 0)
		printf("Log_initialize: error %d initializing log_mutex\n", rc);
...
#if defined(GETTIMEOFDAY)
	snprintf(&msg_buf[22], sizeof(msg_buf)-22, ".%.3llu ", (long long unsigned int)(cur_entry->ts.tv_usec / 1000L));
#else
...
static void Log_output(enum LOG_LEVELS log_level, const char *msg)
{
    if (log_level <= TRACE_MINIMUM)
    {
        wm_log_debug("%s", msg + 19);//19: skip timestamp
    }
    else if (log_level == TRACE_PROTOCOL)
    {
        wm_log_info("%s", msg + 19);//19: skip timestamp
    }
    else if (log_level >= LOG_ERROR)
    {
        wm_log_error("%s", msg + 19);//19: skip timestamp
    }
}
