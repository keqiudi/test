...
#define BUILD_TIMESTAMP __TIME__
...
extern void * SSL_get1_session(void *ssl);
extern int SSL_set_session(void *ssl, void *session);
#endif
...
int MQTTClient_init(void)
{
	...
	if (mqttclient_mutex == NULL)
	{
		...
		{
			rc = GetLastError();
			wm_log_debug("mqttclient_mutex error %d", rc);
			goto exit;
		}
		if ((subscribe_mutex = CreateMutex(NULL, 0, NULL)) == NULL)
		{
			rc = GetLastError();
			wm_log_debug("subscribe_mutex error %d", rc);
			goto exit;
		}
		if ((unsubscribe_mutex = CreateMutex(NULL, 0, NULL)) == NULL)
		{
			rc = GetLastError();
			wm_log_debug("unsubscribe_mutex error %d", rc);
			goto exit;
		}
		if ((connect_mutex = CreateMutex(NULL, 0, NULL)) == NULL)
		{
			rc = GetLastError();
			wm_log_debug("connect_mutex error %d", rc);
			goto exit;
		}
#if !defined(NO_HEAP_TRACKING)
		...
		{
			rc = GetLastError();
			wm_log_debug("stack_mutex error %d", rc);
			goto exit;
		}
		if ((heap_mutex = CreateMutex(NULL, 0, NULL)) == NULL)
		{
			rc = GetLastError();
			wm_log_debug("heap_mutex error %d", rc);
			goto exit;
		}
#endif
		...
		{
			rc = GetLastError();
			wm_log_debug("log_mutex error %d", rc);
			goto exit;
		}
		if ((socket_mutex = CreateMutex(NULL, 0, NULL)) == NULL)
		{
			rc = GetLastError();
			wm_log_debug("socket_mutex error %d", rc);
			goto exit;
		}
	}
exit:
...
int MQTTClient_init(void)
{
	...
	if ((rc = pthread_mutex_init(mqttclient_mutex, &attr)) != 0)
		wm_log_debug("MQTTClient: error %d initializing client_mutex", rc);
	else if ((rc = pthread_mutex_init(socket_mutex, &attr)) != 0)
		wm_log_debug("MQTTClient: error %d initializing socket_mutex", rc);
	else if ((rc = pthread_mutex_init(subscribe_mutex, &attr)) != 0)
		wm_log_debug("MQTTClient: error %d initializing subscribe_mutex", rc);
	else if ((rc = pthread_mutex_init(unsubscribe_mutex, &attr)) != 0)
		wm_log_debug("MQTTClient: error %d initializing unsubscribe_mutex", rc);
	else if ((rc = pthread_mutex_init(connect_mutex, &attr)) != 0)
		wm_log_debug("MQTTClient: error %d initializing connect_mutex", rc);
...
#if defined(OPENSSL) || defined(USE_MBEDTLS)
	int ssl;
#endif
	int websocket;
	Clients* c;
...
start:
	run_goon_flag = 0;
	timeout = 10L; /* first time in we have a small timeout.  Gets things started more quickly */
	...
	while (!tostop)
	{
		...
		Thread_lock_mutex(mqttclient_mutex);
		//Log(TRACE_MED, -1, "%s: pack 0x%x, rc %d", __func__, pack, rc);
		if (tostop){
			break;
		}
		timeout = 100L;
		...
		if (rc == SOCKET_ERROR || rc == TCPSOCKET_INTERRUPTED)
		{
			wm_log_debug("Error, state: %d", m->c->connect_state);
			if (m->c->connected && rc == SOCKET_ERROR){
				wm_log_debug("disconnect");
			...
			else
			{
				...
				if(m->c->connect_state == WEBSOCKET_IN_PROGRESS)
				{
					Log(TRACE_MIN, -1, "Posting wsconnack semaphore for client %s", m->c->clientID);
					m->c->connect_state = NOT_IN_PROGRESS;
					Thread_post_sem(m->connect_sem);
				}
			}
						...
						if (dp)
						{
							...
							if (dp->properties)
							{
								*(dp->properties) = disc->properties;
								...
								#else
								(*(dp->m->disconnected))(dp->m->disconnected_context, dp->properties, dp->reasonCode);
								MQTTProperties_free(dp->properties);
								free(dp->properties);
								free(dp);
								#endif
							}
			...
			else if (m->c->connect_state == SSL_IN_PROGRESS)
			{
				...
				if (rc == 1 || rc == SSL_FATAL)
				{
					if (rc == 1 && (m->c->cleansession == 0 && m->c->cleanstart == 0) && m->c->session == NULL)
						m->c->session = SSL_get1_session((void *)m->c->net.ssl);
					m->rc = rc;
	...
	START_TIME_TYPE start, ELAPSED_TIME_TYPE millisecsTimeout, MQTTProperties* connectProperties, MQTTProperties* willProperties)
{
	...
	Log(TRACE_MED, -1, "%s: m->ma 0x%x, running %d, millisecsTimeout %ld", __func__, m->ma, running, millisecsTimeout);
	if (m->ma && !running)
	{
		int count = 0;
	...
	wm_log_debug("connect out: rc=%d", rc);
...
MQTTResponse MQTTClient_subscribeMany5_wm(MQTTClient handle, int count, char* const* topic,
		int* qos, MQTTSubscribe_options* opts, MQTTProperties* props, int *msg_id)
{
	MQTTClients* m = handle;
	List* topics = NULL;
	List* qoss = NULL;
...
MQTTResponse MQTTClient_subscribe5_wm(MQTTClient handle, const char* topic, int qos,
		MQTTSubscribe_options* opts, MQTTProperties* props, int *msg_id)
{
	MQTTResponse rc;

	FUNC_ENTRY;
	rc = MQTTClient_subscribeMany5_wm(handle, 1, (char * const *)(&topic), &qos, opts, props, msg_id);
	if (qos == MQTT_BAD_SUBSCRIBE) /* addition for MQTT 3.1.1 - error code from subscribe */
		rc.reasonCode = MQTT_BAD_SUBSCRIBE;
	FUNC_EXIT_RC(rc.reasonCode);
	return rc;
...
int MQTTClient_subscribe_wm(MQTTClient handle, const char* topic, int qos, int *msg_id)
{
	MQTTClients* m = handle;
	MQTTResponse response = MQTTResponse_initializer;
	...
	if (m->c->MQTTVersion >= MQTTVERSION_5)
		...
		response = MQTTClient_subscribe5_wm(handle, topic, qos, NULL, NULL, msg_id);
...
MQTTResponse MQTTClient_unsubscribeMany5_wm(MQTTClient handle, int count, char* const* topic, MQTTProperties* props, int *msg_id)
{
	MQTTClients* m = handle;
	List* topics = NULL;
	int i = 0;
...
MQTTResponse MQTTClient_unsubscribe5_wm(MQTTClient handle, const char* topic, MQTTProperties* props, int *msg_id)
{
	MQTTResponse rc;

	rc = MQTTClient_unsubscribeMany5_wm(handle, 1, (char * const *)(&topic), props, msg_id);
	return rc;
}
...
int MQTTClient_unsubscribe_wm(MQTTClient handle, const char* topic, int *msg_id)
{
	MQTTResponse response = MQTTClient_unsubscribe5_wm(handle, topic, NULL, msg_id);
...
static MQTTPacket* MQTTClient_cycle(SOCKET* sock, ELAPSED_TIME_TYPE timeout, int* rc)
{
	...
	FUNC_ENTRY;
	//Log(TRACE_MIN, -1, "%s: timeout %d", __func__, timeout);
#if defined(OPENSSL) || defined(USE_MBEDTLS)
...
#endif
	Thread_lock_mutex(mqttclient_mutex);
	//Log(TRACE_MED, -1, "%s: *sock %d, rc1 %d", __func__, *sock, rc1);
	if (*sock > 0 && rc1 == 0)
	{
		MQTTClients* m = NULL;
		if (ListFindItem(handles, sock, clientSockCompare) != NULL)
...
static MQTTPacket* MQTTClient_waitfor(MQTTClient handle, int packet_type, int* rc, int64_t timeout)
{
	...
	{
		Log(LOG_ERROR, MQTTCLIENT_FAILURE, "%s: waitfor error handle 0x%x, timeout %ld", __func__, handle, timeout);
		*rc = MQTTCLIENT_FAILURE;
		goto exit;
	}
	...
	if (is_running)
	{
		...
		pack = m->pack;
		Log(TRACE_MEDIUM, -1, "client %s, packet_type %d, timeout %ld, pack 0x%x", m->c->clientID, packet_type, timeout, pack);
	}
	else
	{
