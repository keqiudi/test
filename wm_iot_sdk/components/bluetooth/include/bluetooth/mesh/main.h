...
typedef enum bt_mesh_output_auth_action {
	BT_MESH_NO_OUTPUT       = 0,
...
typedef enum bt_mesh_input_auth_action {
	BT_MESH_NO_INPUT      = 0,
...
typedef enum bt_mesh_prov_bearer {
...
typedef enum bt_mesh_prov_oob_info_location {
...
struct bt_mesh_prov {
	 ...
	 *
	 * Remember to enable CONFIG_BT_MESH_PROV_OOB_PUBLIC_KEY
 ...
 *  authentication action. When using BT_MESH_PUSH, BT_MESH_TWIST or
 *  BT_MESH_ENTER_NUMBER, the @ref bt_mesh_prov::output_number callback is
 *  called with a random number that has to be entered on the unprovisioned
 *  device.
 *
 *  When using BT_MESH_ENTER_STRING, the @ref bt_mesh_prov::output_string
 ...
 *  be called.
 *
 *  When using BT_MESH_BLINK, BT_MESH_BEEP, BT_MESH_VIBRATE
 *  or BT_MESH_DISPLAY_NUMBER, and the application has to call
 *  @ref bt_mesh_input_number with the random number indicated by
 *  the unprovisioned device.
 *
 *  When using  BT_MESH_DISPLAY_STRING, the application has to call
