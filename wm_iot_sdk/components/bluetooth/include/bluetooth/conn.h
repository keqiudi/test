 ...
 * Check that @ref bt_conn_info.state is  BT_CONN_STATE_CONNECTED.
 *
 ...
 *        valid data if CONFIG_BT_REMOTE_VERSION is enabled.
 */
 ...
 *  and subversion) CONFIG_BT_REMOTE_VERSION must be enabled
 *
 ...
 *
 *  @return Zero on success or (negative) error code on failure.
 *  @return -ENOBUFS if unable to allocate command buffer.
 */
 ...
 *  CONFIG_BT_CONN_PARAM_UPDATE_TIMEOUT option.
 *
 *  @param conn Connection object.
 ...
 *  Use  BT_GAP_LE_PHY_NONE to indicate no preference.
 *
 *  @param conn Connection object.
 *  @param param Updated connection parameters.
 *
 ...
 *
 *  @note When CONFIG_BT_SMP_SC_ONLY is enabled then the security
 *        level will always be level 4.
 *
 *  @note When CONFIG_BT_SMP_OOB_LEGACY_PAIR_ONLY is enabled then the
...
struct bt_conn_cb {
	 ...
	 *  - BT_HCI_ERR_UNKNOWN_CONN_ID Creating the connection started by
	 *    bt_conn_le_create was canceled either by the user through
	 ...
	 *  advertise or create a new connection using k_work_submit or
	 *  increase CONFIG_BT_MAX_CONN.
	 *
	 *  @param conn Connection object.
...
/** @brief Set authentication mode to be used for pairing.
 *
 *  @param auto_mode true for auto selection, otherwise relys on settings
 *  The initial value of this flag is auto .
 *
 *  @return always 0 on success .
 */
int bt_auth_mode_set(bool auto_mode);

/** @brief Get authentication mode to be used for pairing.
 *
 *  @return bool true auto or relys on settings .
 */
bool bt_auth_mode_get(void);


/** @brief Set authentication value to be used for pairing.
 *
 *  Set the authreq flag in the Authentication Requirements of
 *  SMP Pairing Request/Response data.
 *  The initial value of this flag depends on BT_SMP_AUTH_DEFAULT setting.
 *  @return 0 on success or a negative error code on failure.
 */
int bt_auth_set(uint8_t auth);

/** @brief Get authentication value to be used for pairing.
 *
 *  Get the authreq flag in the Authentication Requirements of
 *  SMP Pairing Request/Response data.
 *  The initial value of this flag depends on BT_SMP_AUTH_DEFAULT setting.
 *  @return smp_auth.
 */
...
/** @brief Set encrypion key size to be used for pairing.
 *
 *  Set the encryptio key size in the Authentication Requirements of
 *  SMP Pairing Request/Response data.
 *  The initial value of this flag depends on CONFIG_BT_SMP_MIN_ENC_KEY_SIZE Kconfig option.
 *  @return 0 on success or a negative error code on failure.
 */
int bt_enc_key_size_set(uint8_t key_size);

/** @brief Get encrypion key size to be used for pairing.
 *
 */
uint8_t bt_enc_key_size_get(void);

/** @brief Set distribution to be used for pairing.
 *
 *  Set the initiator and responder key flag in the Authentication Requirements of
 *  SMP Pairing Request/Response data.
 *  The initial value of this flag depends on SEND_KEYS/RECV_KEYS setting.
 *  @return 0 on success or a negative error code on failure.
 */
...
struct bt_conn_oob_info {
	...
	union {
		...
		struct {
			...
			enum bt_conn_oob_configuration {
 ...
 *  The CONFIG_BT_SMP must be enabled for this function.
 *
