 ...
 *          May also be returned if CONFIG_BT_SMP is enabled and a
 *          pairing procedure is already in progress.
 *
 *  @retval -ENOBUFS Not buffers available to send request to controller or if
 *          CONFIG_BT_SMP is enabled and no more keys could be stored.
 *
 *  @retval -ENOMEM If CONFIG_BT_SMP is enabled and no more keys
 *          could be stored.
 *
 *  @retval -EIO Controller rejected the request or if CONFIG_BT_SMP
 *          is enabled and pairing has timed out.
 *
 *  @retval -ENOTCONN If CONFIG_BT_SMP is enabled the ACL is not
 *          connected.
 */
