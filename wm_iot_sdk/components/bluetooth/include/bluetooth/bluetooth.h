 ...
 *
 * When CONFIG_BT_SETTINGS is enabled, the application must load the
 ...
 *
 * @note Requires CONFIG_BT_DEVICE_NAME_DYNAMIC.
 *
 * @sa CONFIG_BT_DEVICE_NAME_MAX.
 *
 * @param name New name
 *
 * @return Zero on success or (negative) error code otherwise.
 ...
 *
 * This symbol is linkable if CONFIG_BT_DEVICE_APPEARANCE_DYNAMIC is
 * enabled.
 *
...
enum bt_le_adv_option {
	 ...
	 * and/or enabling privacy CONFIG_BT_PRIVACY.
	 */
	 ...
	 * CONFIG_BT_EXT_ADV instead.
	 *
	 * @note Not implemented when CONFIG_BT_PRIVACY.
	 *
...
struct bt_le_adv_param {
	 ...
	 *
	 * @note When extended advertising CONFIG_BT_EXT_ADV is not
	 ...
	 * called with the status set to BT_HCI_ERR_ADV_TIMEOUT.
	 */
 ...
 * the timeout will be  BT_GAP_ADV_HIGH_DUTY_CYCLE_MAX_TIMEOUT.
 *
...
struct bt_le_ext_adv_start_param {
	 ...
	 * to the maximum of  BT_GAP_ADV_HIGH_DUTY_CYCLE_MAX_TIMEOUT.
	 *
	 * If privacy CONFIG_BT_PRIVACY is enabled then the timeout
	 * must be less than CONFIG_BT_RPA_TIMEOUT.
	 */
	uint16_t timeout;
	/**
 ...
 *  BT_GAP_ADV_MAX_ADV_DATA_LEN bytes.
 ...
 * possible of  BT_GAP_ADV_MAX_EXT_ADV_DATA_LEN bytes.
 *
...
struct bt_le_per_adv_sync_cb {
	 ...
	 *              CONFIG_BT_PER_ADV_SYNC_RSP is enabled.
	 */
...
struct bt_le_scan_recv_info {
	 ...
	 *  BT_GAP_ADV_PROP_SCAN_RESPONSE bit.
	 *
	 */
	uint16_t adv_props;
 ...
 *       local device when CONFIG_BT_PRIVACY is disabled. This is to
 ...
 *       CONFIG_BT_SCAN_WITH_IDENTITY must be enabled.
 *
...
/**
 * @brief Check (LE) scanning state.
 *
 *
 * @return LE Scan state. Zero means scan not working,  positive means bt dev is scanning
 *         1 indicates passive scan and 2 means active scan.
 */
 ...
 * with this API.
 *
 * Initially, the if CONFIG_BT_RPA_TIMEOUT is used as the
 * RPA timeout.
 *
 * This symbol is linkable if CONFIG_BT_RPA_TIMEOUT_DYNAMIC
 * is enabled.
 *
 ...
 *
 * If privacy CONFIG_BT_PRIVACY is enabled this will result in
 * generating new Resolvable Private Address (RPA) that is valid for
 * CONFIG_BT_RPA_TIMEOUT seconds. This address will be used for
 ...
 *      In addition when extended advertising CONFIG_BT_EXT_ADV is
 ...
 * If privacy CONFIG_BT_PRIVACY is enabled this will result in
 * generating new Resolvable Private Address (RPA) that is valid for
 * CONFIG_BT_RPA_TIMEOUT seconds. This address will be used by the
 * advertising set.
 *
