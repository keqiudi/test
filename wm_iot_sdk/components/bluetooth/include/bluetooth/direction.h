...
struct bt_df_adv_cte_tx_param {
	 ...
	 * @brief CTE type.
	 *
	 * Allowed values are defined by bt_df_cte_type, except BT_DF_CTE_TYPE_NONE and
	 * BT_DF_CTE_TYPE_ALL.
	 */
	uint8_t  cte_type;
 ...
 * expects ( bt_df_cte_type). In case cte_type bit BT_DF_CTE_TYPE_AOA is not set, members:
...
struct bt_df_conn_cte_tx_param {
	/**
	 * Bitfield with allowed CTE types ( bt_df_cte_type. All enum members may be used except
	 * BT_DF_CTE_TYPE_NONE).
	 */
	uint8_t cte_types;
