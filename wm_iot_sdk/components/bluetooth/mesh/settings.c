...
SETTINGS_STATIC_HANDLER_DEFINE(bt_mesh, "ms", NULL, NULL, mesh_commit,
			       NULL);
...
{
	if (IS_ENABLED(CONFIG_BT_MESH_SETTINGS_WORKQ)) {
        k_work_queue_init(&settings_work_q);
		k_work_queue_start(&settings_work_q, NULL,
				   SETTINGS_WORKQ_STACK_SIZE,
				   K_PRIO_COOP(SETTINGS_WORKQ_PRIO), NULL);
		k_thread_name_set(&settings_work_q.thread, "BT MS workq");
	}
