...
{
	k_work_deinit_delayable(&bt_mesh.ivu_timer);
    k_mem_slab_deinit(&loopback_buf_pool);
}
...
void bt_mesh_net_pending_dev_key_cand_store(void)
{
	...
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_DEVKEY_CAND)) {
		err = settings_save_one("ms/DevKeyC", &bt_mesh.dev_key_cand,
					sizeof(struct bt_mesh_key));
	} else {
		err = settings_delete("ms/DevKeyC");
	}
...
static void store_pending_iv(void)
{
	...
	err = settings_save_one("ms/IV", &iv, sizeof(iv));
	if (err) {
...
static void store_pending_net(void)
{
	...
	err = settings_save_one("ms/Net", &net, sizeof(net));
	if (err) {
...
void bt_mesh_net_pending_seq_store(void)
{
	...
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
		...
		err = settings_save_one("ms/Seq", &seq, sizeof(seq));
		if (err) {
