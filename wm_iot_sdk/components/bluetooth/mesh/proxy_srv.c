...
#define PROXY_SVC_INIT_TIMEOUT K_MSEC(1600)
...
static struct bt_mesh_proxy_client {
	...
	struct k_work_delayable send_beacons;
...
= {
	[0 ... (CONFIG_BT_MAX_CONN - 1)] = {
		.send_beacons = Z_WORK_DELAYABLE_INITIALIZER(proxy_send_beacons),
	},
};
#endif
...
static struct bt_mesh_subnet *beacon_sub = NULL;
...
static void proxy_send_beacons(struct k_work *work)
{
	...
	client = CONTAINER_OF(dwork, struct bt_mesh_proxy_client, send_beacons);
			       ...
			       const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	if (client->filter_type == NONE) {
		...
		k_work_schedule(&client->send_beacons, K_MSEC(1000));
	}
...
static struct k_work_delayable svc_reg_work;
...
int bt_mesh_proxy_gatt_init(void)
{
    LOG_DBG("");

    k_work_init_delayable(&svc_reg_work, svc_reg_work_handler);

    for(int i = 0; i < CONFIG_BT_MAX_CONN; i++)
    {
        k_work_init_delayable(&clients[i].send_beacons, proxy_send_beacons);
    }

    return 0;
}

int bt_mesh_proxy_gatt_deinit(void)
{
    LOG_DBG("");

    k_work_deinit_delayable(&svc_reg_work);

    for(int i = 0; i < CONFIG_BT_MAX_CONN; i++)
    {
        k_work_deinit_delayable(&clients[i].send_beacons);
    }
...
static void gatt_connected(struct bt_conn *conn, uint8_t err)
{
	...
	LOG_INF("proxy service gatt connected conn %p err 0x%02x", (void *)conn, err);
...
static void gatt_disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	LOG_INF("proxy service gatt disconnected conn %p reason 0x%02x", (void *)conn, reason);
