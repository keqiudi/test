		...
		uint8_t *p, uint8_t out[32])
{
	...
	err = bt_mesh_sha256_hmac_one_raw_key(t, p, (size_t)strlen((const char *)p), out);
	if (err) {
		return err;
	}
		...
		const uint8_t *conf_salt, uint8_t *conf_key)
{
	...
	if (algorithm == BT_MESH_PROV_AUTH_CMAC_AES128_AES_CCM &&
		...
		return bt_mesh_k1(k_input, 32, conf_salt, (const char *)"prck", conf_key);
	}
