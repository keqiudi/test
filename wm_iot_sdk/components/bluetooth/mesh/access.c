			    ...
			    uint8_t *mod_cnt, struct net_buf_simple *buf)
{
	...
	ext_mod_cnt = count_mod_ext(mod, (uint8_t *)&max_offset);
          ...
          bool vnd, bool primary, void *user_data)
{
    ...
    if (mod->pub) {
         mod->pub->mod = NULL;
         k_work_deinit_delayable(&mod->pub->timer);
    }
			...
			const struct bt_mesh_send_cb *cb, void *cb_data)
{
	...
	LOG_DBG("access out:net_idx 0x%04x app_idx 0x%04x dst 0x%04x", tx.ctx->net_idx, tx.ctx->app_idx,
		tx.ctx->addr);
			      ...
			      struct bt_mesh_elem *elem, uint32_t opcode)
{
	...
	if (!bt_mesh_model_has_key(model, ctx->app_idx)) {
		LOG_ERR("Wrong key, model %p, app_idx %d", model, ctx->app_idx);
...
int bt_mesh_model_recv(struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf)
{
	...
	LOG_DBG("access in:app_idx 0x%04x src 0x%04x dst 0x%04x", ctx->app_idx, ctx->addr,
		ctx->recv_dst);
	LOG_DBG("len %u: %s", buf->len, bt_hex(buf->data, buf->len));
			    ...
			    const char *key, char *path, size_t path_len)
{
	...
	if (vnd) {
		snprintk(path, path_len, "ms/v/%x/%s", mod_key, key);
	} else {
		snprintk(path, path_len, "ms/s/%x/%s", mod_key, key);
	}
}
...
int bt_mesh_models_metadata_store(void)
{
	...
	err = settings_save_one("ms/metadata", buf.data, buf.len);
	if (err) {
...
int bt_mesh_models_metadata_read(struct net_buf_simple *buf, size_t offset)
{
	...
	err = settings_load_subtree_direct("ms/metadata", read_comp_cb, &stored_buf);
	if (err) {
...
static void models_metadata_clear(void)
{
	...
	err = settings_delete("ms/metadata");
	if (err) {
