...
static struct k_fifo bt_mesh_adv_queue;
static struct k_fifo bt_mesh_relay_queue;
static struct k_fifo bt_mesh_friend_queue;
...
struct net_buf *bt_mesh_create_unbusy_buf(void)
{
    struct net_buf *buf;

    buf = bt_mesh_adv_create(BT_MESH_ADV_BEACON, 0, 0,
					 K_NO_WAIT);
    if (!buf) {
        LOG_WRN("Out of update adv buffers\n");
        return NULL;
    }
...
int bt_mesh_adv_buf_init(void)
{
    k_fifo_init(&bt_mesh_adv_queue);
    k_fifo_init(&bt_mesh_relay_queue);
    k_fifo_init(&bt_mesh_friend_queue);
    net_buf_pool_init(&adv_buf_pool, "adv_buf_pool", CONFIG_BT_MESH_ADV_BUF_COUNT*2,
		    BT_MESH_ADV_DATA_SIZE, BT_MESH_ADV_USER_DATA_SIZE,
		    adv_buf_destroy);

#if defined(CONFIG_BT_MESH_RELAY)
    net_buf_pool_init(&relay_buf_pool, "relay_buf_pool", CONFIG_BT_MESH_RELAY_BUF_COUNT,
                BT_MESH_ADV_DATA_SIZE, BT_MESH_ADV_USER_DATA_SIZE,
                adv_buf_destroy);
#endif

#if defined(CONFIG_BT_MESH_ADV_EXT_FRIEND_SEPARATE)
    net_buf_pool_init(&friend_buf_pool,"friend_buf_pool", CONFIG_BT_MESH_FRIEND_LPN_COUNT,
		    BT_MESH_ADV_DATA_SIZE, BT_MESH_ADV_USER_DATA_SIZE,
		    adv_buf_destroy);
#endif
...
int bt_mesh_adv_buf_deinit(void)
{
    k_fifo_deinit(&bt_mesh_adv_queue);
    k_fifo_deinit(&bt_mesh_relay_queue);
    k_fifo_deinit(&bt_mesh_friend_queue);
    net_buf_pool_deinit(&adv_buf_pool);

#if defined(CONFIG_BT_MESH_RELAY)
    net_buf_pool_deinit(&relay_buf_pool);
#endif

#if defined(CONFIG_BT_MESH_ADV_EXT_FRIEND_SEPARATE)
    net_buf_pool_deinit(&friend_buf_pool);
#endif
