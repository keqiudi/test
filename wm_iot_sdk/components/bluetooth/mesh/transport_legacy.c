...
void bt_mesh_trans_reset(void)
{
	...
	bt_mesh_rpl_clear();
	bt_mesh_va_clear();
	k_mem_slab_deinit(&segs);
	k_mem_slab_init(&segs, NULL, BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS);
}
...
void bt_mesh_trans_deinit(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(seg_tx); i++) {
		k_work_deinit_delayable(&seg_tx[i].retransmit);
	}

	for (i = 0; i < ARRAY_SIZE(seg_rx); i++) {
		k_work_deinit_delayable(&seg_rx[i].ack);
	}
