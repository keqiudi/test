			...
			struct bt_mesh_adv *adv)
{
	...
	uint16_t duration;
	int err;
    int32_t adv_int_min = ADV_INT_FAST_MS;

    if (CONFIG_BT_CTRL_VERSION >= 42)
    {
...
static void adv_buf_cancel_work_handler(struct k_work *work)
{
    (void)work;
    bt_mesh_adv_buf_get_cancel();
}
static struct k_work_delayable adv_buf_cancel_work;
...
{
    k_work_init_delayable(&adv_buf_cancel_work, adv_buf_cancel_work_handler);

	k_thread_create(&adv_thread_data, NULL,
			CONFIG_BT_MESH_ADV_STACK_SIZE, adv_thread,
			NULL, K_PRIO_COOP(CONFIG_BT_MESH_ADV_PRIO),
			0, K_FOREVER);
	k_thread_name_set(&adv_thread_data, "Mesh adv");
}

void bt_mesh_adv_deinit(void)
{
    k_thread_delete(&adv_thread_data);
    k_work_deinit_delayable(&adv_buf_cancel_work);
}
