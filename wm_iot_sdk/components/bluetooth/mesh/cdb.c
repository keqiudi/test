...
static void store_cdb_node(const struct bt_mesh_cdb_node *node)
{
	...
	snprintk(path, sizeof(path), "ms/cdb/Node/%x", node->addr);
...
static void clear_cdb_node(uint16_t addr)
{
	...
	snprintk(path, sizeof(path), "ms/cdb/Node/%x", addr);
...
static void store_cdb_subnet(const struct bt_mesh_cdb_subnet *sub)
{
	...
	snprintk(path, sizeof(path), "ms/cdb/Subnet/%x", sub->net_idx);
...
static void clear_cdb_subnet(uint16_t net_idx)
{
	...
	snprintk(path, sizeof(path), "ms/cdb/Subnet/%x", net_idx);
	err = settings_delete(path);
	if (err) {
...
static void store_cdb_app_key(const struct bt_mesh_cdb_app_key *app)
{
	...
	snprintk(path, sizeof(path), "ms/cdb/AppKey/%x", app->app_idx);
...
static void clear_cdb_app_key(uint16_t app_idx)
{
	...
	snprintk(path, sizeof(path), "ms/cdb/AppKey/%x", app_idx);
	err = settings_delete(path);
	if (err) {
...
static void clear_cdb_net(void)
{
	...
	err = settings_delete("ms/cdb/Net");
	if (err) {
...
static void store_cdb_pending_net(void)
{
	...
	err = settings_save_one("ms/cdb/Net", &net, sizeof(net));
	if (err) {
