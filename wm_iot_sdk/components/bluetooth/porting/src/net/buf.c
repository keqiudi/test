...
static sys_slist_t net_buf_pool_list;

void net_buf_pool_init(struct net_buf_pool *buf_pool,const char*name, uint16_t count, size_t data_size, size_t ud_size, destroy_cb_t destroy)
{
    if(buf_pool == NULL) return;
    
    k_lifo_init(&buf_pool->free);

    struct net_buf_pool_fixed *fixed = (struct net_buf_pool_fixed *)k_malloc(sizeof(struct net_buf_pool_fixed));
    __ASSERT_NO_MSG(fixed);
    fixed->data_size = data_size;
    fixed->data_pool = (uint8_t*)k_malloc(count*data_size);
    __ASSERT_NO_MSG(fixed->data_pool);
    
    struct net_buf_data_alloc *alloc = (struct net_buf_data_alloc *)k_malloc(sizeof(struct net_buf_data_alloc));
    __ASSERT_NO_MSG(alloc);
    alloc->cb = &net_buf_fixed_cb;
    alloc->alloc_data = (void*)fixed;

    buf_pool->alloc = alloc;
    buf_pool->__bufs = (struct net_buf*)k_malloc(count *(sizeof(struct net_buf)+ud_size));
    __ASSERT_NO_MSG(buf_pool->__bufs);
    buf_pool->buf_count = count;
    buf_pool->uninit_count = count;
    buf_pool->user_data_size = ud_size;
#if defined(CONFIG_NET_BUF_POOL_USAGE)
    buf_pool->avail_count = ATOMIC_INIT(count);
    buf_pool->name = name;
#endif
    buf_pool->destroy = destroy;

    sys_slist_append(&net_buf_pool_list, &buf_pool->node);

}
void net_buf_pool_deinit(struct net_buf_pool *buf_pool)
{
    if(buf_pool == NULL) return;
    
    bool removed =false;
    struct net_buf_pool_fixed *fixed = NULL;
    struct net_buf_data_alloc *alloc = buf_pool->alloc;
    
    k_lifo_deinit(&buf_pool->free);

    removed = sys_slist_find_and_remove(&net_buf_pool_list, &buf_pool->node);
    __ASSERT_NO_MSG(removed);
    (void)removed;
    
    if(buf_pool->__bufs) {
        k_free(buf_pool->__bufs);
        buf_pool->__bufs = NULL;
    }
    
    if(alloc){
        fixed = alloc->alloc_data;
        if(fixed){
            if(fixed->data_pool){
                k_free(fixed->data_pool);
                fixed->data_pool = NULL;
            }
            k_free(fixed);
            fixed = NULL;
        }
...
#else
    SYS_SLIST_FOR_EACH_NODE(&net_buf_pool_list, node) {
        if(index == id) break;
        index++;
    }
    pool  = CONTAINER_OF(node, struct net_buf_pool, node);
#endif
	return pool;
}

int pool_id(struct net_buf_pool *pool)
{
#if 0
...
#endif
{
	...
	NET_BUF_DBG("%s():%d: pool %p size %zu count %zu uninit count %zu", func, line, pool, size, pool->buf_count,pool->uninit_count);
    //printf(": pool %s %p size %d count %d uninit count %d\r\n",pool->name, pool, size, pool->buf_count,pool->uninit_count);
