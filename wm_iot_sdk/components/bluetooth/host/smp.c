...
static bool smp_auth_mode = true;
static uint8_t smp_auth = BT_SMP_AUTH_DEFAULT;
static uint8_t smp_enc_key_size = BT_SMP_MIN_ENC_KEY_SIZE;
static uint8_t smp_send_key = SEND_KEYS;
static uint8_t smp_recv_key = RECV_KEYS;
static uint8_t smp_send_key_sc = SEND_KEYS_SC;
static uint8_t smp_recv_key_sc = RECV_KEYS_SC;
...
static void smp_pairing_complete(struct bt_smp *smp, uint8_t status)
{
	...
	if(status && IS_ENABLED(CONFIG_BT_DISCONNECT_AFTER_SMP_ERROR) && conn->role == BT_HCI_ROLE_CENTRAL) {
...
#ifdef BT_SMP_AUTO_MODE
	if(smp_auth_mode == false) {
		return (auth & smp_auth);
	}
#endif
...
static int smp_send_security_req(struct bt_conn *conn)
{
	...
	req = net_buf_add(req_buf, sizeof(*req));
#ifdef BT_SMP_AUTO_MODE
	req->auth_req = get_auth(smp, smp_auth);
#else
...
#ifdef BT_SMP_AUTO_MODE
		(req->max_key_size < smp_enc_key_size))
#else
...
#ifdef BT_SMP_AUTO_MODE
	rsp->init_key_dist = (req->init_key_dist & smp_recv_key);
	rsp->resp_key_dist = (req->resp_key_dist & smp_send_key);

	if ((rsp->auth_req & BT_SMP_AUTH_SC) &&
	    (req->auth_req & BT_SMP_AUTH_SC)) {
		atomic_set_bit(smp->flags, SMP_FLAG_SC);

		rsp->init_key_dist &= smp_recv_key_sc;
		rsp->resp_key_dist &= smp_send_key_sc;
	}
#else
...
static int smp_send_pairing_req(struct bt_conn *conn)
{
...
#ifdef BT_SMP_AUTO_MODE
		req->init_key_dist = smp_send_key;
		req->resp_key_dist = smp_recv_key;
#else
    ...
    LOG_INF("auth_req:0x%02x, init_key:0x%02x, resp_key:0x%02x", req->auth_req, req->init_key_dist, req->resp_key_dist);
...
#ifdef BT_SMP_AUTO_MODE
	(rsp->max_key_size < smp_enc_key_size))
#else
...
#ifdef BT_SMP_AUTO_MODE
	smp->local_dist &= smp_send_key_sc;
	smp->remote_dist &= smp_recv_key_sc;
#else
	smp->local_dist &= SEND_KEYS_SC;
	smp->remote_dist &= RECV_KEYS_SC;
#endif
...
static void bt_smp_disconnected(struct bt_l2cap_chan *chan)
{
    ...
    k_work_deinit_delayable(&smp->work);
				  ...
				  uint8_t hci_status)
{
    ...
    LOG_DBG("1smp->remote_dist 0x%02x local_dist 0x%02x", smp->remote_dist, smp->local_dist);
    ...
    LOG_DBG("2smp->remote_idst 0x%02x, local_dist 0x%02x", smp->remote_dist,smp->local_dist);
...
{
    if(init_key & 0xF0 || resp_key & 0xF0)
    {
        return -EINVAL;
    }

    smp_send_key = init_key;
    smp_recv_key = resp_key;

    /**NOTE, to make at+bleaddr work, CONFIG_BT_PRIVACY is disabled;
       which means the smp can not distribute BT_SMP_DIST_ID_KEY. So,
       we force to disable BT_SMP_DIST_ID_KEY even if it is configured in this API*/
    ///if(smp_auth_mode == true)
    {
#if !defined(CONFIG_BT_PRIVACY)
        smp_send_key &= ~ BT_SMP_DIST_ID_KEY;
        smp_recv_key &= ~ BT_SMP_DIST_ID_KEY;
#endif
    }

    smp_send_key_sc = smp_send_key;
    smp_recv_key_sc = smp_recv_key;

    smp_send_key_sc &= ~BT_SMP_DIST_ENC_KEY;
    smp_recv_key_sc &= ~BT_SMP_DIST_ENC_KEY;
...
int bt_enc_key_size_set(uint8_t key_size)
{
    ...
    smp_enc_key_size = key_size;
    return 0;
}

int bt_auth_mode_set(bool auto_mode)
{
    smp_auth_mode = !auto_mode;
    return 0;
}

bool bt_auth_mode_get(void)
{
    return !smp_auth_mode;
}
...
int bt_auth_set(uint8_t auth)
{
    ...
    /*mapping api auth definition to smp level*/
    switch(auth)
    {
        case 0:
            smp_auth = BT_SMP_AUTH_NONE;
            break;
        case 1:
            smp_auth = BT_SMP_AUTH_BONDING;
            break;
        case 4:
            smp_auth = BT_SMP_AUTH_MITM;
            break;
        case 8:
            smp_auth = BT_SMP_AUTH_SC;
            break;
        case 9:
            smp_auth = BT_SMP_AUTH_SC | BT_SMP_AUTH_BONDING;
            break;
        case 12:
            smp_auth = BT_SMP_AUTH_SC | BT_SMP_AUTH_MITM;
            break;
        case 13:
            smp_auth = BT_SMP_AUTH_SC | BT_SMP_AUTH_MITM | BT_SMP_AUTH_BONDING;
            break;
        default:
            LOG_WRN("illegal auth req param");
            ret = -EINVAL;
            break;
    }
...
{
    k_sem_deinit(&sc_local_pkey_ready);
}
int bt_smp_init(void)
{
	...
	LOG_INF("LE SC %s", sc_supported ? "enabled" : "disabled");
    
    err = k_sem_init(&sc_local_pkey_ready, 0, 1);
    if(err) {
        return err;
    }
