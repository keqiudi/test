...
void bt_l2cap_chan_del(struct bt_l2cap_chan *chan)
{
	...
	LOG_DBG("bt_l2cap_chan_del:conn %p chan %p destroy %p cid %d", chan->conn, chan, chan->destroy, le_chan->rx.cid);
...
destroy:
    ...
    k_work_deinit_delayable(&le_chan->rtx_work);
#endif
	if (chan->destroy) {
		chan->destroy(chan);
	}
...
static void l2cap_chan_destroy(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	if (k_current_get() != le_chan->rtx_work.work_q->thread.task) {
    ...
    k_work_deinit_delayable(&le_chan->tx_work);

    k_fifo_deinit(&le_chan->tx_queue);
...
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
    k_fifo_deinit(&free_l2cap_tx_meta_data);
#endif
...
{
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
    net_buf_pool_init(&disc_pool, "disc_pool", 1,
			  BT_L2CAP_BUF_SIZE(
				sizeof(struct bt_l2cap_sig_hdr) +
				sizeof(struct bt_l2cap_disconn_req)),
			  8, NULL);
