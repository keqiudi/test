			    ...
			    const bt_addr_le_t *addr, const char *key)
{
	if (key) {
		snprintk(path, path_size,
			 "%s/%02x%02x%02x%02x%02x%02x%u/%s", subsys,
			 addr->a.val[5], addr->a.val[4], addr->a.val[3],
			 addr->a.val[2], addr->a.val[1], addr->a.val[0],
			 addr->type, key);
	} else {
		snprintk(path, path_size,
			 "%s/%02x%02x%02x%02x%02x%02x%u", subsys,
			 addr->a.val[5], addr->a.val[4], addr->a.val[3],
			 addr->a.val[2], addr->a.val[1], addr->a.val[0],
			 addr->type);
	}
	...
	if (!strncmp(name, "id", len)) {
		...
		if (len < sizeof(bt_dev.id_addr[0])) {
			if (len < 0) {
				...
				LOG_HEXDUMP_DBG((uint8_t*)&bt_dev.id_addr, len, "data read");
			}
...
SETTINGS_STATIC_HANDLER_DEFINE(bt, "", NULL, set_setting, commit_settings, NULL);
...
{
    settings_subsys_deinit_resource();
		      ...
		      size_t val_len)
{
	...
	if (addr) {
		...
		err = snprintk(key_str, sizeof(key_str), "%s", key);
		if (err < 0) {
			return -EINVAL;
		}
	}
...
{
#if defined(CONFIG_BT_PRIVACY)
	int err = bt_settings_store("i", 0, NULL, bt_dev.irk, ID_DATA_LEN(bt_dev.irk));
