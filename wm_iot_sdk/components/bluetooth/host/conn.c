...
static void bt_conn_free(struct bt_conn *conns)
{
#if defined(CONFIG_BT_CONN)
        k_work_deinit_delayable(&conns->deferred_work);
#endif /* CONFIG_BT_CONN */
#if defined(CONFIG_BT_CONN_TX)
        k_work_init(&conns->tx_complete_work, NULL);
#endif /* CONFIG_BT_CONN_TX */
...
static struct bt_conn_tx *conn_tx_alloc(void)
{
	 ...
	 */
	if (k_current_get() == k_sys_work_q.thread.task) {
			       ...
			       struct k_poll_event *events)
{
	...
	if ((conn->state == BT_CONN_DISCONNECTED || conn->state == BT_CONN_CONNECTING_SCAN) &&
...
void bt_conn_process_tx(struct bt_conn *conn)
{
    ...
    if(err != 0) LOG_WRN("bt_conn_process_tx send_buf ret %d[ENOTCONN=%d]", err, -ENOTCONN);
    //if(err == -ENOTCONN) net_buf_unref(buf);
	if (err  == -EIO) {
		struct bt_conn_tx *tx = tx_data(buf)->tx;
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	conn->state = state;
	LOG_DBG("s conn %p, %s -> %s [%d]", conn, state2str(old_state), state2str(conn->state), wm_os_internal_get_time());
    ...
    LOG_DBG("e conn %p, %s -> %s [%d]", conn, state2str(old_state), state2str(conn->state), wm_os_internal_get_time());
}
...
static void notify_connected(struct bt_conn *conn)
{
    ...
    {
#if defined(CONFIG_BT_CONN)
	    k_work_deinit_delayable(&conn->deferred_work);
#endif /* CONFIG_BT_CONN */
    }
}
...
static void deferred_work(struct k_work *work)
{
	...
	LOG_DBG("deferred_work: conn %p state %s\n", conn,state2str(conn->state));
	if (conn->state == BT_CONN_DISCONNECTED || conn->state == BT_CONN_CONNECTING_SCAN) {
#if defined(CONFIG_BT_ISO_UNICAST)
		struct bt_conn *iso;
...
int bt_conn_set_security(struct bt_conn *conn, bt_security_t sec)
{
	...
	if (conn->sec_level >= sec || conn->required_sec_level >= sec) {
        LOG_WRN("sec level already satisfied");
		return 0;
	}
...
void bt_conn_deinit(void)
{
#if defined(CONFIG_BT_CONN)
    net_buf_pool_deinit(&acl_tx_pool);
#if CONFIG_BT_L2CAP_TX_FRAG_COUNT > 0
    net_buf_pool_deinit(&frag_pool);
#endif
#endif

    k_fifo_deinit(&free_tx);
    bt_att_deinit();
    bt_smp_deinit();
    bt_l2cap_deinit();
...
#if defined(CONFIG_BT_CONN)
    net_buf_pool_init(&acl_tx_pool,"acl_tx_pool", CONFIG_BT_L2CAP_TX_BUF_COUNT,
		    BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU),
		    CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);
#if CONFIG_BT_L2CAP_TX_FRAG_COUNT > 0
    net_buf_pool_init(&frag_pool, "frag_pool", CONFIG_BT_L2CAP_TX_FRAG_COUNT,
			  BT_BUF_ACL_SIZE(CONFIG_BT_BUF_ACL_TX_SIZE),
			  CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);
#endif
#endif
