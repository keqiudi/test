...
static int process_queue(struct bt_att_chan *chan, struct k_fifo *queue)
{
	...
	if (buf) {
		...
		if (err) {
            ...
            LOG_WRN("bt_att_chan_send failed, push back the buf %p except for err %d [%d]", buf, err,EINVAL);
            if(1){
                LOG_WRN("Free buf, id %d", buf->pool_id);
                net_buf_unref(buf);
            }else{
...
static void bt_att_connected(struct bt_l2cap_chan *chan)
{
	...
	LOG_DBG("bt_att_connected chan %p cid 0x%04x", le_chan, le_chan->tx.cid);
...
static void bt_att_disconnected(struct bt_l2cap_chan *chan)
{
    ...
    k_work_deinit_delayable(&att_chan->timeout_work);
...
static void bt_att_status(struct bt_l2cap_chan *ch, atomic_t *status)
{
	...
	LOG_DBG("chan %p status %p value %08x", ch, status, status[0]);
...
static int bt_att_accept(struct bt_conn *conn, struct bt_l2cap_chan **ch)
{
	...
	LOG_DBG("bt_att_accept conn %p handle %u", conn, conn->handle);
	...
	att_handle_rsp_thread->task = k_current_get();
...
{
    k_fifo_deinit(&free_att_tx_meta_data);
    
#if CONFIG_BT_ATT_PREPARE_COUNT > 0
    net_buf_pool_deinit(&prep_pool);
#endif
    bt_gatt_deinit();
...
void bt_att_init(void)
{
    ...
    k_mem_slab_init(&att_slab, NULL, sizeof(struct bt_att),CONFIG_BT_MAX_CONN);
    k_mem_slab_init(&chan_slab,NULL, sizeof(struct bt_att_chan), CONFIG_BT_MAX_CONN * ATT_CHAN_MAX);
    k_mem_slab_init(&req_slab, NULL, sizeof(struct bt_att_req), CONFIG_BT_L2CAP_TX_BUF_COUNT);
#if CONFIG_BT_ATT_PREPARE_COUNT > 0
    net_buf_pool_init(&prep_pool, "prep_pool", CONFIG_BT_ATT_PREPARE_COUNT, BT_ATT_BUF_SIZE,
    		    sizeof(struct bt_attr_data), NULL);
...
struct bt_att_req *bt_att_req_alloc(k_timeout_t timeout)
{
	...
	if (k_current_get() == att_handle_rsp_thread->task) {
