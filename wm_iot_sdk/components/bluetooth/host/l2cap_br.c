...
static void l2cap_br_chan_destroy(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	if (k_current_get() != br_chan->rtx_work.work_q->thread) {
...
void bt_l2cap_br_deinit(void)
{
    ...
    net_buf_pool_deinit(&br_sig_pool);
}
void bt_l2cap_br_init(void)
{
	sys_slist_init(&br_servers);
    
    net_buf_pool_init(&br_sig_pool, "br_sig_pool", CONFIG_BT_MAX_CONN,
			  BT_L2CAP_BUF_SIZE(L2CAP_BR_MIN_MTU), 8, NULL);
