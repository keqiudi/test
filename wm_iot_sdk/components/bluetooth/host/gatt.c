...
static int bt_gatt_store_cf(uint8_t id, const bt_addr_le_t *peer)
{
	...
	LOG_HEXDUMP_DBG((uint8_t*)str, len, "Saved data");
#endif /* CONFIG_BT_GATT_CACHING */
	return 0;
...
{
	if (atomic_test_and_clear_bit(gatt_flags, GATT_INITIALIZED)) {
		atomic_clear_bit(gatt_flags, GATT_SERVICE_INITIALIZED);
        last_static_handle = 0;
#if defined(CONFIG_BT_GATT_CACHING)
        k_work_deinit_delayable(&db_hash.work);
#endif
#if defined(CONFIG_BT_GATT_SERVICE_CHANGED)
        k_work_deinit_delayable(&gatt_sc.work);
    if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
...
#endif
#if defined(CONFIG_BT_SETTINGS_DELAYED_STORE)
        k_work_deinit_delayable(&gatt_delayed_store.work);
#endif

	}
    
#if defined(CONFIG_BT_GATT_DYNAMIC_DB)
    k_mutex_deinit(&db_lock);
#endif
...
int bt_gatt_service_register(struct bt_gatt_service *svc)
{
	...
	k_mutex_lock(111, &db_lock, K_FOREVER);
		   ...
		   void *cb_arg)
{
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		LOG_DBG("Restored CCC for id:0x%02x addr:%s", load.addr_with_id.id,
...
BT_SETTINGS_DEFINE(ccc, "c", ccc_set_cb, NULL);
			  ...
			  void *cb_arg, void *param)
{
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		if (!settings_name_steq((const char *)param, "c", &name)) {
			LOG_ERR("Invalid key");
			return -EINVAL;
		}
...
BT_SETTINGS_DEFINE(sc, "s", sc_set, sc_commit);
#endif /* CONFIG_BT_GATT_SERVICE_CHANGED */
...
BT_SETTINGS_DEFINE(cf, "f", cf_set, NULL);
