...
#define LOG_TAG "bt_hci_core"

#define CONFIG_BT_HCI_RX_THREAD
...
#if defined(CONFIG_BT_HCI_RX_THREAD)
			 ...
			 struct net_buf **rsp)
{
	...
	if (!buf) {
		buf = bt_hci_cmd_create(opcode, 0);
		if (!buf) {
			LOG_ERR("hci_cmd_create opcode 0x%04x failed", opcode);
			return -ENOBUFS;
		}
	}

	LOG_DBG("buf %p opcode 0x%04x len %u timestack %d", buf, opcode, buf->len, wm_os_internal_get_time_ms());
	k_sem_init(&sync_sem, 0, 1);
	cmd(buf)->sync = &sync_sem;

	net_buf_put(&bt_dev.cmd_tx_queue, net_buf_ref(buf));
	k_sys_pool_sem_give(2);

	err = k_sem_take(&sync_sem, HCI_CMD_TIMEOUT);
	
	if(err != 0) LOG_ERR("command opcode 0x%04x timeout with err %d timestack %d", opcode, err, wm_os_internal_get_time_ms());
	//BT_ASSERT_MSG(err == 0, "command opcode 0x%04x timeout with err %d", opcode, err);

	k_sem_deinit(&sync_sem);
...
static void conn_handle_disconnected(uint16_t handle)
{
	for (int i = 0; i < ARRAY_SIZE(disconnected_handles); i++) {
		if (!disconnected_handles[i]) {
			 ...
			 */
			LOG_WRN("conn_handle_disconnected handle %d set", handle);
...
static void hci_disconn_complete_prio(struct net_buf *buf)
{
	...
	LOG_DBG("hci_disconn_complete_prio status 0x%02x handle %u reason 0x%02x", evt->status, handle, evt->reason);
			...
			const struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	conn->role = evt->role;
	conn->err = 0U;
    memset(conn->le.peer_features, 0, sizeof(conn->le.peer_features));
...
void bt_hci_le_enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
    ...
    LOG_DBG("bt_conn_connected, conn %p, state %d , err %d", conn, conn->state, conn->err);
	bt_conn_connected(conn);
    
    LOG_DBG("conn_auto_initiate, conn %p", conn);
...
static void le_remote_feat_complete(struct net_buf *buf)
{
	...
	if (!evt->status) {
        ...
        memcpy(conn->le.peer_features, evt->features,
		       sizeof(conn->le.features));
	}else
    {
        memset(conn->le.peer_features, 0,
		       sizeof(conn->le.features));
    }   
...
static void hci_encrypt_change(struct net_buf *buf)
{
	...
	conn = bt_conn_lookup_handle(handle, BT_CONN_TYPE_ALL);
	if (!conn) {
		LOG_ERR("ENC CHANGE Unable to look up conn with handle %u", handle);
		return;
	}
...
static void hci_encrypt_key_refresh_complete(struct net_buf *buf)
{
	...
	conn = bt_conn_lookup_handle(handle, BT_CONN_TYPE_ALL);
	if (!conn) {
		LOG_ERR("HCI ENC KEY REFRERSH Unable to look up conn with handle %u", handle);
		return;
	}
...
static void hci_cmd_complete(struct net_buf *buf)
{
	...
	LOG_DBG("opcode 0x%04x ncmd %d", opcode, ncmd);
...
static void hci_rx_thread(void *arg)
{
	...
	while(1)
	{
		buf = k_queue_get(&bt_dev.krx_queue, K_FOREVER);

		if(!buf) {
			continue;
		}

		LOG_DBG("hci_rx_thread GET buf %p type %u len %u", buf, bt_buf_get_type(buf), buf->len);
...
static void rx_queue_put(struct net_buf *buf)
{
    ...
    status = k_queue_append(&bt_dev.krx_queue, buf);
    if(status !=0 )
	{
		LOG_WRN("preappend krx queue failed, from pool id %d", buf->pool_id);
		net_buf_unref(buf);
	}
    return;
#else
	...
	const int err = k_work_submit_to_queue(0, &bt_workq, &rx_work);
#endif /* CONFIG_BT_RECV_WORKQ_SYS */
	if (err < 0) {
		LOG_ERR("Could not submit rx_work: %d flags: 0x%08x", err, rx_work.flags);
	}
#endif
}
#endif /* !CONFIG_BT_RECV_BLOCKING */

int bt_pre_recv(struct net_buf *buf)
{
	int status;
...
int bt_recv(struct net_buf *buf)
{
	...
	LOG_DBG("buf %p len %u buf type %u", buf, buf->len, bt_buf_get_type(buf));
	...
	case BT_BUF_EVT:
	{
        ...
        LOG_DBG("evt_flags:%02x", evt_flags);
...
static int bt_init(void)
{
    ...
    net_buf_pool_init(&hci_cmd_pool, "hci_cmd_pool", CONFIG_BT_BUF_CMD_TX_COUNT, CMD_BUF_SIZE, 9, NULL);
...
#if 0
	buf = k_queue_get(&bt_dev.krx_queue, K_NO_WAIT);
...
#endif
	LOG_DBG("GET buf %p type %u len %u", buf, bt_buf_get_type(buf), buf->len);
...
#elif defined(CONFIG_BT_RECV_WORKQ_BT)
		err = k_work_submit_to_queue(1, &bt_workq, &rx_work);
#endif
		if (err < 0) {
			LOG_ERR("impossible.........rx_work_handler Could not submit rx_work: %d", err);
        }
	}
}
#endif /* !CONFIG_BT_RECV_BLOCKING */
...
int bt_enable(bt_ready_cb_t cb)
{
...
#endif    
	k_queue_init(&bt_dev.krx_queue);

	atomic_clear_bit(bt_dev.flags, BT_DEV_DISABLE);

	if (atomic_test_and_set_bit(bt_dev.flags, BT_DEV_ENABLE)) {
	     LOG_WRN("BT_DEV_ENABLE already");
		return -EALREADY;
	}
	...
	/* TX thread */
	k_thread_create(&tx_thread_data, NULL,
...
#if defined(CONFIG_BT_HCI_RX_THREAD)
    /* HCI RX thread*/
	k_thread_create(&rx_thread_data, NULL,
			CONFIG_BT_RX_STACK_SIZE,
			hci_rx_thread, NULL,
			K_PRIO_COOP(CONFIG_BT_HCI_TX_PRIO - 1),
			0, K_NO_WAIT);
	...
	k_work_queue_start(&bt_workq, NULL,
			   CONFIG_BT_RX_STACK_SIZE,
			   K_PRIO_COOP(CONFIG_BT_RX_PRIO), NULL);
	k_thread_name_set(&bt_workq.thread, "BT WORKER");
#endif
...
int bt_disable(void)
{
	...
	k_queue_deinit(&bt_dev.krx_queue);

	k_sem_deinit(&bt_dev.ncmd_sem);

	k_fifo_deinit(&bt_dev.cmd_tx_queue);

	net_buf_pool_deinit(&hci_cmd_pool);
