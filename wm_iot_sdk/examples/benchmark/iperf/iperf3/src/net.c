...
Nwrite(int fd, const char *buf, size_t count, int prot)
{
    register ssize_t r;
    register size_t nleft = count;
    uint32_t time_inms = wm_os_internal_get_time();
    uint32_t time_oums;
	...
	if (r < 0) {
	    ...
	    case ENOMEM:
#if 1
	        time_oums = wm_os_internal_get_time();
            if (time_oums - time_inms > 3000) {
                printf("iperf mem not enough\n");
                time_inms = wm_os_internal_get_time();
            }
