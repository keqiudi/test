...
static struct bt_uuid_128 read_characteristic_uuid =
    BT_UUID_INIT_128(BT_UUID_128_ENCODE(0x12345678, 0x1234, 0x5678, 0x1234, 0x56789abcdef1));

static struct bt_uuid_128 write_characteristic_uuid =
...
BT_GATT_SERVICE_DEFINE(primary_service, BT_GATT_PRIMARY_SERVICE(&primary_service_uuid),
                       BT_GATT_CHARACTERISTIC(&read_characteristic_uuid.uuid, BT_GATT_CHRC_READ, BT_GATT_PERM_READ, read_signed,
                                              NULL, NULL),
                       BT_GATT_CHARACTERISTIC(&write_characteristic_uuid.uuid, BT_GATT_CHRC_WRITE, BT_GATT_PERM_WRITE_ENCRYPT,
                                              NULL, write_signed, NULL));

static const struct bt_data ad[] = { BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)) };

static const struct bt_data sd[] = { BT_DATA_BYTES(BT_DATA_UUID128_ALL, BT_UUID_CUSTOM_SERVICE_VAL) };
...
static void connected(struct bt_conn *conn, uint8_t err)
{
    ...
    if (err) {
        ...
        wm_log_info("Connected with %s", addr);
    }
}
...
static void add_bonded_addr_to_filter_list(const struct bt_bond_info *info, void *data)
{
    ...
    wm_log_info("Added %s to advertising accept filter list", addr_str);
    bond_count++;
}
...
extern void wm_system_reboot(void);
void pairing_complete(struct bt_conn *conn, bool bonded)
{
    wm_log_info("Pairing completed. Rebooting in 5 seconds...");

    k_sleep(K_SECONDS(5));
    wm_system_reboot();
}

static struct bt_conn_auth_info_cb bt_conn_auth_info = { .pairing_complete = pairing_complete };
