...
#define LOG_TAG "peripheral_csc"
#include "wm_log.h"

#define CSC_SUPPORTED_LOCATIONS                                                                         \
    {                                                                                                   \
        CSC_LOC_OTHER, CSC_LOC_FRONT_WHEEL, CSC_LOC_REAR_WHEEL, CSC_LOC_LEFT_CRANK, CSC_LOC_RIGHT_CRANK \
    }
...
static ssize_t read_location(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, uint16_t len, uint16_t offset)
{
...
static ssize_t read_csc_feature(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, uint16_t len, uint16_t offset)
{
...
static void ctrl_point_ind(struct bt_conn *conn, uint8_t req_op, uint8_t status, const void *data, uint16_t data_len);
                                ...
                                uint16_t offset, uint8_t flags)
{
    ...
    switch (req->op) {
        ...
        case SC_CP_OP_REQ_SUPP_LOC:
            ...
            ctrl_point_ind(conn, req->op, SC_CP_RSP_SUCCESS, &supported_locations, sizeof(supported_locations));
...
BT_GATT_SERVICE_DEFINE(csc_svc, BT_GATT_PRIMARY_SERVICE(BT_UUID_CSC),
                       BT_GATT_CHARACTERISTIC(BT_UUID_CSC_MEASUREMENT, BT_GATT_CHRC_NOTIFY, 0x00, NULL, NULL, NULL),
                       BT_GATT_CCC(csc_meas_ccc_cfg_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
                       BT_GATT_CHARACTERISTIC(BT_UUID_SENSOR_LOCATION, BT_GATT_CHRC_READ, BT_GATT_PERM_READ, read_location,
                                              NULL, &sensor_location),
                       BT_GATT_CHARACTERISTIC(BT_UUID_CSC_FEATURE, BT_GATT_CHRC_READ, BT_GATT_PERM_READ, read_csc_feature, NULL,
                                              NULL),
...
static void measurement_nfy(struct bt_conn *conn, uint32_t cwr, uint16_t lwet, uint16_t ccr, uint16_t lcet)
{
    struct csc_measurement_nfy *nfy;
    uint8_t buf[sizeof(*nfy) + (cwr ? sizeof(struct wheel_rev_data_nfy) : 0) + (ccr ? sizeof(struct crank_rev_data_nfy) : 0)];
    uint16_t len = 0U;
...
static void csc_simulation(void)
{
    ...
    if (!(i % 2)) {
        lwet += 1050 + rand_32 % 50;
        c_wheel_revs += 2U;
        nfy_wheel = true;
    }

    if (!(i % 3)) {
        lcet += 1000 + rand_32 % 50;
        ccr += 1U;
        nfy_crank = true;
    }
	 ...
	 * by the Client.
	 */
    measurement_nfy(NULL, nfy_wheel ? c_wheel_revs : 0, nfy_wheel ? lwet : 0, nfy_crank ? ccr : 0, nfy_crank ? lcet : 0);
